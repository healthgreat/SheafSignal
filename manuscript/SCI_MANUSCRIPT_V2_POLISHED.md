...
cell-cell communication as a higher-rank ligand-receptor-channel sheaf over a
biological graph and uses Hodge decomposition to quantify edge-level
frustration, directional signal, local curl-like structure and global
circulation-like graph components.
Controlled simulations recovered gradient, curl, harmonic and mixed-flow
components, and task-based perturbation simulations were used to separate
method recovery from residual-defined ground truth. In public
tumor-microenvironment benchmarks spanning melanoma,
...
Comparisons with ligand-receptor product scoring, LIANA, bounded
nichenetr-engine scoring and a mechanistic target-prior baseline showed
alignment with existing communication evidence while preserving a distinct
inconsistency-focused readout. Task-based synthetic comparisons also showed
that simple flow-gradient product baselines can match SheafSignal on current
perturbation tasks, so broad superiority over all scalar baselines is not
claimed.
...
analysis. For each directed sender-receiver edge, the method represents
valid ligand-receptor channels as coordinates in a higher-rank vertex stalk.
Sender and receiver restrictions are scaled by ligand and receptor expression
for each channel, and the per-channel mismatch between observed communication
and expected pathway-state transition is treated as sheaf energy, or
communication frustration. The resulting edge flow is then decomposed using graph Hodge
...
LR-channel coordinates. For each channel, communication evidence is derived
from sender ligand and receiver receptor expression, while the expected flow is
defined by expression-scaled restriction maps acting on sender and receiver
pathway states. Edge-level sheaf energy is the aggregated standardized
mismatch across active LR channels. Node-level frustration scores summarize
the share of outgoing inconsistency contributed by each sender population,
while global gradient/curl/harmonic ratios summarize the structure of the
resulting flow.
...
### Simulations and task-based comparisons validate implementation boundaries
...
To address the risk of residual-defined ground truth, we added an independent
perturbation task in which positive edges were predefined perturbation edges
rather than edges selected by the SheafSignal residual. In this task,
SheafSignal and the higher-rank LR-channel sheaf score recovered the predefined
edges, but a simple flow-gradient opposition product baseline also matched the
current task. We therefore use this experiment to reduce circularity and to
set claim boundaries, not to claim universal superiority over scalar baselines.
...
recognized communication evidence. The task-based comparator panel further
separates independent perturbation recovery from residual-aligned supporting
controls. The intended conclusion is complementarity: existing tools prioritize
communication evidence, whereas SheafSignal asks whether those graph-level
flows are consistent with pathway-state structure.
...
rather than intensity alone. By representing communication as a higher-rank
LR-channel sheaf and decomposing the resulting residual flow with Hodge theory,
the method reports where communication edges are discordant with pathway-state
transitions and whether the fitted graph-flow structure is dominated by
directional, local curl-like or global circulation-like components. This provides an
...
Several limitations are central to the current study. First, task-based
synthetic comparisons currently show that a flow-gradient product baseline can
tie SheafSignal on simple perturbation recovery tasks; stronger task designs
are required before claiming algorithmic superiority over all scalar baselines.
Second, all disease
examples are based on public data and should be interpreted as computational
and hypothesis-generating. Third, some annotations are coarse, especially in
GSE154778 and the Visium case, so cell-type claims require explicit QC and
should not be overextended. Fourth, the current comparator set does not exhaust
all CCC tools or all possible NicheNet resources. Fifth, SheafSignal itself
...
### Higher-rank LR-channel sheaf and sheaf energy

For each directed sender-receiver pair, sender ligand expression and receiver
receptor expression define channel-specific communication evidence. The
vertex stalk is `R^m`, with one coordinate per valid ligand-receptor channel.
For each edge and channel, sender and receiver restriction coefficients are
scaled by ligand and receptor expression, and these maps act on sender and
receiver pathway states to define the expected communication flow. Observed
and expected channel flows are standardized before their residual is
calculated. Aggregated channel residual energy is reported as edge-level sheaf
energy, with sender-level frustration scores computed from outgoing sheaf
energy. The original rank-one scalar residual is retained only as a legacy
compatibility output and is not treated as the main novelty claim.
