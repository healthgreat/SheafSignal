...
- External beta-review packet generation is implemented by `scripts/build_external_beta_review_packet.py`; the current decision is `BETA_REVIEW_PACKET_READY_INTERNAL_AI_REVIEWS_TRIAGED_EXTERNAL_HUMAN_REVIEWS_PENDING`, meaning internal multi-agent AI critiques are filed but independent external human review remains pending.
- Internal multi-agent AI reviews have now been returned and triaged in `external_ai_review_packet/ROUND3_RETURNED_REVIEW_TRIAGE_2026-05-02.md`; stale findings are separated from still-open blockers, but independent human or outside-lab review remains pending.
...
- Local clean-export reproduction preflight is implemented by `scripts/run_clean_clone_preflight.py`; the current decision is `CLEAN_CLONE_PREFLIGHT_PASS_LOCAL_EXPORT` for commit `73cc44d`, covering core pytest, CLI demo, expected output checks, and release audit from a Git-tracked export.
...
- `benchmarks/results/confirmatory_10000/confirmatory_permutation_subset.tsv`
