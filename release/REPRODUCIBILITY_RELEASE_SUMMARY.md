# SheafSignal Reproducibility Release Summary

## Release Boundary

This repository is prepared for a public GitHub + Zenodo release. GitHub should
contain source code, manifests, environment files, tests, documentation, and
small benchmark result tables. Zenodo should contain frozen processed benchmark
objects and large generated spatial benchmark tables.

## Current Inventory

- GitHub-target files: 635
- GitHub-target size MB: 30.617
- Zenodo-target files: 45
- Zenodo-target size MB: 445.122

## Generated Files

- GitHub manifest: `release/github_release_manifest.tsv`
- Zenodo manifest: `release/zenodo_upload_manifest.tsv`
- GitHub SHA256 sums: `release/github_sha256sums.txt`
- Zenodo SHA256 sums: `release/zenodo_sha256sums.txt`
- Zenodo deposition metadata: `release/zenodo_deposition_metadata.json`
- Zenodo deposition instructions: `release/ZENODO_DEPOSITION_INSTRUCTIONS.md`

## Claim Boundary

No DOI has been minted by this script. The manuscript can state that the
release package is Zenodo-ready after these manifests are generated, but it
should not cite a Zenodo DOI until an actual deposition is created and the DOI
is inserted into `metadata/datasets.tsv` or the manuscript data availability
statement.
